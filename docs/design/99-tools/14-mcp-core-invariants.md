...
- **默认串联自动修（可关闭）**：`run_game_basic_test_flow` 与 `run_game_basic_test_flow_by_current_state` 默认在流程未通过时，在 **`max_repair_rounds`**（1–8，默认 2）与 **`auto_fix_max_cycles`**（默认 3，0 表示跳过 `auto_fix` 调用）上限内先尝试 **`remediation_matrix`** 对应的 **handler**（若有），再按需调用 **`auto_fix_game_bug`** 与再次验证。传 **`auto_repair: false`** 或设置环境变量 **`GPF_AUTO_REPAIR_DEFAULT=0`** 可恢复「仅跑流程、不自动修」。以下错误码**不**进入修复环，仍直接抛 `AppError`：`INVALID_ARGUMENT`、`TEMP_PROJECT_FORBIDDEN`、`INVALID_GODOT_PROJECT`、`BROADCAST_ENTRY_REQUIRED`。`FLOW_GENERATION_BLOCKED` 进入修复环时优先走「刷新上下文并重试流程生成」类 handler，仍失败则继续 `auto_fix` 或耗尽轮次后失败返回。
- **AI 代理会话与 `GPF_AUTO_REPAIR_DEFAULT=0`：** 当 shell 继承 CI 的 **`GPF_AUTO_REPAIR_DEFAULT=0`** 但代理仍希望默认开启外层自动修复时，在未传 **`auto_repair`** 的前提下设置 **`GPF_AGENT_SESSION_DEFAULTS=1`** 或传 **`agent_session_defaults: true`**；显式 **`auto_repair: false`** 仍优先关闭。CI **不得**设置 `GPF_AGENT_SESSION_DEFAULTS`。详见 **`docs/gpf-ai-agent-integration.md`**。
- **失败处理契约 `failure_handling`：** 推荐集成方传 **`failure_handling: "run_only" | "auto_try_fix"`**（与用户 AskQuestion 选项 id 一致），在未传 **`auto_repair`** 时决定外层是否自动修复；非法值 **`INVALID_ARGUMENT`**。详见 **`docs/design/99-tools/17-basic-flow-failure-handling-contract.md`**。
