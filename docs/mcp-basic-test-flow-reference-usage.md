...
5. **默认自动修复闭环**：上述两个 run 工具默认 **`auto_repair: true`**（可用 **`auto_repair: false`** 或环境变量 **`GPF_AUTO_REPAIR_DEFAULT=0`** 关闭）。成功或结束后，结果中可能含 **`auto_repair`** 字段（`final_status`、`rounds` 等）。仍可直接调用 **`auto_fix_game_bug`** 做独立诊断修复。
