...
- `stopGameTestSession` (**alias** of `closeProject`, same bridge semantics; prefer in new flows to avoid the word “project”)
...
- **What `closeProject` does:** it requests an end to the **F5 / debug play session** (the running game under test, including a floating window whose title often contains debug markers), matching the human workflow **“stop the game, keep the editor open to fix bugs”**. Mechanically: the runtime bridge writes `pointer_gpf/tmp/auto_stop_play_mode.flag`; the **EditorPlugin** consumes it and calls **`EditorInterface.stop_playing_scene()`** (plus a deferred second check if Play is still active), same family as the editor **Stop** button. It does **not** quit the Godot editor application unless your integration adds a separate quit path.
...
- **Plugin failure artifact:** if the editor plugin still observes `EditorInterface.is_playing_scene()` after internal stop retries, it may write `pointer_gpf/tmp/teardown_debug_game_last.json` (`schema: pointer_gpf.teardown_debug_game.v1`, `ok: false`, `reason`). MCP `project_close` may include `debug_game_teardown_ok` / `debug_game_teardown_reason` when this file is present.
