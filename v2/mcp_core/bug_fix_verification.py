    ...
    *,
    ...
    if apply_status not in {"fix_applied", "already_aligned"}:
        payload = {
            ...
            "round_id": str(apply_payload.get("round_id", "")).strip(),
            "bug_id": str(apply_payload.get("bug_id", "")).strip(),
            "bug_source": str(apply_payload.get("bug_source", "pre_existing")).strip() or "pre_existing",
            "injected_bug_kind": str(apply_payload.get("injected_bug_kind", "")).strip(),
            "bug_case_file": str(apply_payload.get("bug_case_file", "")).strip(),
