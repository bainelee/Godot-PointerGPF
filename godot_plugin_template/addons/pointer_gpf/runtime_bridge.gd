...
func _perform_click(target: Variant, fallback_pos: Vector2) -> Dictionary:
    ...
    if node != null:
        if node is BaseButton:
            ...
            _dispatch_click_virtual(_control_center(btn))
            return {"ok": true, "node_path": str(btn.get_path())}
        if node is Control:
            var ctrl := node as Control
            _dispatch_click_virtual(_control_center(ctrl))
            return {"ok": true, "node_path": str(ctrl.get_path())}
...
func _request_stop_play_mode() -> void:
    ...
    if out == null:
        return
    out.store_string(JSON.stringify({"requested_by": "runtime_bridge"}))
    out.close()
